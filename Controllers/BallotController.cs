...
namespace SEMS.Controllers
{
    public class BallotController : Controller
    {
        ...
        public IActionResult ManageCandidates()
        {
            ...
            qry += "LEFT JOIN SYMBOLS AS S ON (P.SID = S.SID OR C.SID=S.SID) WHERE CONST_CODE=" + md.constCode;
            ds = dm.create_dataset(qry);
        ...
        public IActionResult AddCandidate(CandidateModel md)
        {
            ...
            qry += md.pstCode + "," + md.constCode + ",'" + md.cName + "','" + md.cNameV1 + "','" + md.gender + "'," + pacode;
            qry += "," + ind + "," + sid + ")";
            dm.runquery(qry);
